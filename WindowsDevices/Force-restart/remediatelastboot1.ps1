...
if (-not (Test-Path $LogDir)) {
    try {
        ...
        # Script will continue, Write-Log will use fallback
    }
}

# Verify we can write to the log file
try {
    Add-Content -Path $LogPath -Value "=== Script started ===" -Force -ErrorAction Stop
} catch {
    Write-Warning "Cannot write to log file at $LogPath - logging will output to console only"
}
